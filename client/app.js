...
app.service("api", ["$http", function($http) {
	...
	this.Workflow.query = function(page) {
		return $http.get(window.location.href + "/api/workflows", { params: { page: page } })
			.then(function(res) {
				return res.data;
			});
	};
	...
	this.Workflow.get = function(id) {
		return $http.get(window.location.href + "/api/workflows/" + id)
			.then(function(res) {
				return res.data;
			});
	};
	...
	this.Workflow.save = function(workflow) {
		return $http.post(window.location.href + "/api/workflows/" + workflow._id, workflow)
			.then(function(res) {
				return res.data;
			});
	};
	...
	this.Workflow.launch = function(id) {
		return $http.post(window.location.href + "/api/workflows/" + id + "/launch");
	};

	this.Workflow.resume = function(id) {
		return $http.post(window.location.href + "/api/workflows/" + id + "/resume");
	};

	this.Workflow.cancel = function(id) {
		return $http.post(window.location.href + "/api/workflows/" + id + "/cancel");
	};

	this.Workflow.log = function(id) {
		return $http.get(window.location.href + "/api/workflows/" + id + "/log")
			.then(function(res) {
				return res.data;
			});
	};
	...
	this.Workflow.remove = function(id) {
		return $http.delete(window.location.href + "/api/workflows/" + id);
	};

	this.Task = {};

	this.Task.query = function(page) {
		return $http.get(window.location.href + "/api/tasks", { params: { page: page } })
			.then(function(res) {
				return res.data;
			});
	};
	...
	this.Task.get = function(id) {
		return $http.get(window.location.href + "/api/tasks/" + id)
			.then(function(res) {
				return res.data;
			});
	};
...
app.controller("WorkflowCtrl", ["$scope", "$interval", "$route", "alert", "api", "FileUploader", function($scope, $interval, $route, alert, api, FileUploader) {
	...
	$scope.uploader = new FileUploader({
		 url: window.location.href + "/api/workflows/" + $route.current.params.id + "/upload"
	});
